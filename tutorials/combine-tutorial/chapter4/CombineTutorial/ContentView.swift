...
struct ContentView: View {
    var body: some View {        
        NavigationView {
            List {
                NavigationLink(destination: CombineZipStreamView(navigationBarTitle: "Zip", description: "Publishers.Zip(publisher1, publisher2)", comparingPublisher: self.zipPublisher)) {
                    Text("Zip")
                }
